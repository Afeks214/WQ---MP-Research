...
_SOURCE_FILES = (
    ...
    "weightiz_shared_feature_store.py",
    "weightiz_feature_tensor_cache.py",
    "weightiz_dtype_guard.py",
    "weightiz_system_logger.py",
    "risk_engine.py",
)
...
def _read_sources(root: Path, overrides: dict[str, str] | None = None) -> dict[str, str]:
    ...
    for rel in _SOURCE_FILES:
        if rel in overrides:
            out[rel] = str(overrides[rel])
            continue
        p = root / rel
        if not p.exists():
            raise RuntimeError(f"SELF_AUDIT_SOURCE_MISSING:{rel}")
