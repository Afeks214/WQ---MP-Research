...
from weightiz_module2_core import Module2Config, _rolling_median_mad_causal, precompute_market_physics, run_weightiz_profile_engine


def _state_from_minute_series(T: int, A: int = 4):
...
def _state_from_daily_points(S: int, A: int = 4):
    ...
    cfg = EngineConfig(T=S, A=A, B=240, tick_size=np.full(A, 0.01, dtype=np.float64))
    st = preallocate_state(ts_ns=ts_ns, cfg=cfg, symbols=tuple(f"A{i}" for i in range(A)))
    return st
