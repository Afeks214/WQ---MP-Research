...
def test_cross_universe_support_short_rejected():
    ...
    scored = build_cross_universe_comparable_scores(finalist_scores=df, config=make_test_config())
...
def test_cross_universe_calendar_mismatch_blocks():
    ...
    with pytest.raises(Module6ValidationError):
        build_cross_universe_comparable_scores(finalist_scores=df, config=make_test_config())
