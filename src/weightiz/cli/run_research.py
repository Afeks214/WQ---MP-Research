...
def build_parser(*, description: str = "Weightiz V3.5 research runner") -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument("--config", required=True, help="Path to YAML config")
    parser.add_argument("--project-root", default=None, help="Override repository root for path resolution")
    parser.add_argument("--artifacts-root", default=None, help="Override artifact root for this run")
...
def _write_launcher_record(
    *,
    ...
    artifacts_root: Path,
    mode: str,
    module6_enabled: bool,
) -> Path:
    record = {
        "run_id": str(run_id),
        "mode": str(mode),
        "module6_enabled": bool(module6_enabled),
    ...
    latest_path = paths.logs_root / "latest_run.json"
    run_path = paths.logs_root / f"{run_id}.json"
    _artifact_write_json(latest_path, record)
    _artifact_write_json(run_path, record)
    return run_path
...
def run_from_namespace(
    ...
    *,
    ...
    logger = get_logger(f"weightiz.cli.{mode}")
    ...
    if args.artifacts_root:
        cfg.harness.report_dir = str(paths.artifacts_root)
    ...
    run_preflight_validation_suite(
        ...
        context={
            ...
            "report_dir": str(resolve_repo_path(cfg.harness.report_dir, project_root=project_root)),
        },
    )
    ...
    summary["launcher_record"] = str(
        _write_launcher_record(
            ...
            artifacts_root=artifacts_root,
            mode=mode,
            module6_enabled=enable_module6,
        )
    )
    _artifact_write_json(run_dir / "run_summary.json", summary)
...
def main(argv: list[str] | None = None) -> None:
    ...
    run_from_namespace(args, enable_module6=True, mode="research")
