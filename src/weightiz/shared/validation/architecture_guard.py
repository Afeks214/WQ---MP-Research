...
def _resolve(root: Path, logical_name: str) -> Path:
    rel = _PATHS[logical_name]
    return root / rel
...
def _assert_sweep_stub(root: Path) -> None:
    path = _resolve(root, "sweep_runner.py")
    src = _load(path)
...
def _assert_run_research_dispatch(root: Path) -> None:
    path = _resolve(root, "run_research.py")
    src = _load(path)
...
def _assert_worker_pipeline(root: Path) -> None:
    path = _resolve(root, "weightiz_module5_harness.py")
    src = _load(path)
...
def _assert_module4_signal_only(root: Path) -> None:
    path = _resolve(root, "weightiz_module4_strategy_funnel.py")
    src = _load(path)
...
def _assert_risk_engine(root: Path) -> None:
    path = _resolve(root, "risk_engine.py")
    src = _load(path)
